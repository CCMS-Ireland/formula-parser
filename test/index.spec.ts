import { getTokenNodes, evaluateTokenNodes } from '../src/index';
...
const getReferenceValue = (referenceName: string) => {
  ...
  if (referenceNameLowerCase === 'estimation') {
    ...
    referenceNameLowerCase.startsWith('n:') ||
    referenceNameLowerCase.startsWith('s:')
  ) {
...
tests.push([
  'if(0.1 + 0.2 = 0.3, "correct math", "incorrect math")',
  'correct math',
]);
tests.push([
  'uppercase(if(max({n:5} ^ 2 - 3, 20, {n:17}, 30 / 4) < 16, "here", "there"))',
  'THERE',
]);
...
tests.push([
  'if({estimation} > 2, "Huge Estimation", "Small Estimation")',
  'Huge Estimation',
]);
