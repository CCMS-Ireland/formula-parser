...
import {
  ...
  fixOperatorsAtTheBegining,
} from '../src/operatorPrecedence';
...
describe('OperatorPrecedence', () => {
  test.each(tests)('%s should be wrapped as %s', (formula, result) => {
    const tokens = applyOperatorPrecedence(
      fixOperatorsAtTheBegining(getTokens(formula))
    );
    expect(
      tokens
...
describe('FixOperatorsAtTheBegining', () => {
  test.each(beginingTests)(
    '%s should be prepended with 0 as %s',
    (formula, result) => {
